...
int programmers::모음사전(string word)
{
    int cnt = -1;
    int answer = 0;
    string target = "";
...
int programmers::타겟넘버(vector<int> numbers, int target)
{
    int answer = 0;
...
int programmers::네트워크(int n, vector<vector<int>> computers)
{
    int answer = 0;
    // 자신을 제외한 1을 찾아서 visited에 true처리
...
int programmers::가장많이받은선물(vector<string> friends, vector<string> gifts)
{
	int answer = 0;
    ...
    // gift 를 space 기준으로 주고받은 사람 구분하기
